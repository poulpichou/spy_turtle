...
class FaceLibrary:
    def __init__(self):
        ...
        with (config_dir/"sequences.json").open(encoding="utf-8") as file:
            self.sequences=json.load(file)
        self.eyes={
            name:self._load_eye(get_asset("eyes",name)["path"])
            for name in get_asset_names("eyes")
        }

    def _load_eye(self,path):
        with Path(path).open(encoding="utf-8") as file:
            return [
                line for raw_line in file
                if (line:=raw_line.strip()) and not line.startswith("size:")
            ]
